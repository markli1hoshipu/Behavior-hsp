...
class EEFBaseTeleop:
    ...
    def __init__(
        ...
        # Store last captured object pose (position, quaternion, source) for easy access
        self.last_object_pose: Optional[Tuple[th.Tensor, th.Tensor, str]] = None
    ...
    def get_last_object_pose(self):
        """Get the last captured object pose from SAM-6D or Contact-GraspNet.

        Returns:
            Tuple[torch.Tensor, torch.Tensor, str] or None: (position, quaternion, source)
            if a pose has been captured, None otherwise.
        """
        return self.last_object_pose

    def print_last_object_pose(self):
        """Print the last captured object pose to console."""
        if self.last_object_pose is None:
            print("\n[Object Pose] No object pose captured yet. Press H to capture.")
            return None

        pos, quat, source = self.last_object_pose
        print("\n" + "="*60)
        print(f"[Object Pose - {source.upper()}]")
        print(f"Position (base frame): {pos}")
        print(f"Quaternion (x,y,z,w): {quat}")
        print("="*60 + "\n")
        return self.last_object_pose

    def capture_object_pose(self):
        """Trigger object pose capture using the configured grasp provider (SAM-6D or Contact-GraspNet).

        This is equivalent to pressing the H key. The captured pose will be stored in
        self.last_object_pose and can be retrieved with get_last_object_pose().
        """
        if self._grasp_provider == "sam6d":
            self._trigger_sam6d_capture()
        elif self._grasp_provider == "contact":
            self._trigger_contact_capture()
        else:
            print("[Capture] No grasp provider configured.")
    ...
    def step(self) -> th.Tensor:
        ...
        if self._pending_sam6d_pose_base is not None:
            ...
            # Store the object pose for easy access
            self.last_object_pose = (pos_target.clone(), quat_target.clone(), source)
            self._pending_sam6d_pose_base = None
...
def main():
    ...
    try:
        while True:
            ...
            # EXAMPLE: Access the last captured object pose from SAM-6D/Contact-GraspNet
            # When you press H, the object pose is automatically stored in teleop.last_object_pose
            # You can access it anytime in the main loop like this:
            #
            # Method 1: Programmatically trigger capture (equivalent to pressing H)
            # teleop.capture_object_pose()
            #
            # Method 2: Get the last captured pose
            # object_pose = teleop.get_last_object_pose()
            # if object_pose is not None:
            #     pos, quat, source = object_pose
            #     print(f"Object position: {pos}")
            #     print(f"Object quaternion: {quat}")
            #     print(f"Source: {source}")
            #
            # Method 3: Simply print it
            # teleop.print_last_object_pose()
            ...
            # ============================================================
            # PLANNER SECTION: Uses object pose from SAM-6D if available
            # ============================================================
            # To capture object pose before running the planner:
            # 1. Press 'H' key during teleoperation to trigger SAM-6D capture
            # 2. Or programmatically call: teleop.capture_object_pose()
            # 3. Wait for the pose to arrive and be processed
            # 4. The pose will be automatically used by the planner below
            # ============================================================
            ...
            # Get object pose from SAM-6D/Contact-GraspNet if available
            object_pose = teleop.get_last_object_pose()
            if object_pose is not None:
                pos, quat, source = object_pose
                # Convert tensors to numpy/list for the planner
                pG_left = pos.detach().cpu().numpy().tolist()
                # Extract quaternion as [x, y, z, w] for scipy
                quat_xyzw = quat.detach().cpu().numpy().tolist()
                print(f"[Planner] Using object pose from {source.upper()}: pos={pG_left}, quat={quat_xyzw}")
            else:
                # Fallback to hardcoded values if no object pose available
                pG_left = [1.82, 0.1, 0.75]
                quat_xyzw = None  # Will use forward kinematics result
                print("[Planner] No object pose available, using hardcoded target")
            ...
            # Use object quaternion if available, otherwise use FK result
            if object_pose is not None and quat_xyzw is not None:
                rG_left = (rot_offset * Rot.from_quat(quat_xyzw)).as_quat().tolist()
            else:
