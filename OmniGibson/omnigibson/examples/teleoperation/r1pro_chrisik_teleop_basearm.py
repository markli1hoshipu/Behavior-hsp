    ...
    def step(self) -> th.Tensor:
        ...
        if self._pending_sam6d_pose_base is not None:
            ...
            try:
                ...
                quat_target = th.tensor(Rot.from_euler("xyz", [30, np.pi*-0.5, 40.0]).as_quat())
                pos_target = th.tensor([0.6, 0.55,  1.1])
                pos_target_r = th.tensor([1.0, -0.55,  0.4])
        ...
        return actions_out, self.ik.planner, q_targets
...
def main():
    ...
    def get_q():
        return teleop.robot.get_joint_positions()[6:]
    ...
    action = 30
    duration = 10
    actions = planner.move_base(
        duration, pos_target, quat_target, pos_target_r, quat_target, get_q()
    )
