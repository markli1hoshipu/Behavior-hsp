...
class ChrisIKAdapter:
    ...
    def solve_to_joint_targets(
        ...
        print("[Test 8.3] ChrisIKAdapter.solve_to_joint_targets() called")
        ...
        print("[Test 8.4] Parsing target poses")
        pG_left, rG_left = parse_target(target_pose_base_left)
        pG_right, rG_right = parse_target(target_pose_base_right)

        print("[Test 8.5] Getting current joint state")
        q = self.get_current_joint_state()
        # breakpoint()
        print("[Test 8.6] Building IK config")
        config = {
            "pG_left": pG_left,
            "pG_right": pG_right,
            "rG_left": rG_left,
        ...
        print("[Test 8.7] Resetting IK solver with config")
        self.ik.reset(config)
        print("[Test 8.8] Calling IK solve")
        if self.ik.solve():
        ...
        print("[Test 8.9] Computing joint solution")
        qsol = q + self.dt * dq
        # breakpoint()
...
class EEFBaseTeleop:
    ...
    def __init__(
        ...
        # from b1k.nav_plan import NavPlanner2 as NavPlanner
        from b1k.planner.nav import NavigationPlanner

        self.nav_plan = NavigationPlanner(30)
        # self.nav_plan.lin_scale=0.7856314804400293

        self.ik = ChrisIKAdapter(robot)
    ...
    def step(self) -> th.Tensor:
        ...
        if self._pending_sam6d_pose_base is not None:
            ...
            try:
                pos_target = th.tensor([ 0.45, -0.2,  0.5])
                pos_target_r = th.tensor([ -0.2, -0.2,  0.5])
                quat_target = th.tensor([ 3.2746e-05, -4.3976e-04, -7.5015e-02,  9.9718e-01])
        ...
        if self._pending_contact_pose_base is not None:
            ...
            print("------------------------------------------")
            self.desired[arm_name] = (pos_target.clone(), quat_target.clone())
            # self.desired['left'] = (tensor([-0.0421,  0.2492,  0.7166]), tensor([ 3.2746e-05, -4.3976e-04, -7.5015e-02,  9.9718e-01]))
            # self.desired['right'] = (tensor([-0.0421,  0.2492,  0.7166]), tensor([ 3.2746e-05, -4.3976e-04, -7.5015e-02,  9.9718e-01]))
            self._target_dirty = True
            self._set_grasp_viz_pose(arm_name, pos_target, quat_target, source)
            self._pending_drive_target = (
                pos_target.clone(),
        ...
        try:
            ...
            print("\n" + "="*80)
            print("[IK] Solving IK - End Effector Poses")
            print("="*80)

            # Print detailed pose information for each arm
            for arm_name in ["left", "right"]:
                print(f"\n{arm_name.upper()} ARM:")
                print("-" * 40)

                # Get current EE pose
                cur_pos, cur_quat = self._get_eef_pose(arm_name)
                cur_pos_np = cur_pos.detach().cpu().numpy()
                cur_quat_np = cur_quat.detach().cpu().numpy()

                # Get target EE pose
                tgt_pos, tgt_quat = self.desired[arm_name]
                tgt_pos_np = tgt_pos.detach().cpu().numpy()
                tgt_quat_np = tgt_quat.detach().cpu().numpy()

                # Calculate differences
                pos_diff = tgt_pos_np - cur_pos_np

                print(f"Current Position: [{cur_pos_np[0]:+.4f}, {cur_pos_np[1]:+.4f}, {cur_pos_np[2]:+.4f}]")
                print(f"Target Position:  [{tgt_pos_np[0]:+.4f}, {tgt_pos_np[1]:+.4f}, {tgt_pos_np[2]:+.4f}]")
                print(f"Difference (T-C): [x: {pos_diff[0]:+.4f}, y: {pos_diff[1]:+.4f}, z: {pos_diff[2]:+.4f}]")
                print(f"Distance Error:   {np.linalg.norm(pos_diff):.4f} m")
                print()
                print(f"Current Quaternion (xyzw): [{cur_quat_np[0]:+.4f}, {cur_quat_np[1]:+.4f}, {cur_quat_np[2]:+.4f}, {cur_quat_np[3]:+.4f}]")
                print(f"Target Quaternion (xyzw):  [{tgt_quat_np[0]:+.4f}, {tgt_quat_np[1]:+.4f}, {tgt_quat_np[2]:+.4f}, {tgt_quat_np[3]:+.4f}]")

            print("="*80 + "\n")

            # Compute global IK and plan
            ...
            pG_left, rG_left = parse_target(self.desired["left"])
            pG_right, rG_right = parse_target(self.desired["right"])

            qc = self.ik.get_current_joint_state()
            ...
            # self.global_ik.joint_names

            qsol = self.global_ik.get_solution()

            print("GOAL POSITION:", pG_left)
            fk = self.ik.ik.left_arm_chain.forward_kinematics({n: qsol[n] for n in self.ik.ik.left_arm_chain.get_joint_parameter_names()})
            ...
            ntargs = 80#75000# increasse number to slow robot
            t = np.linspace(0, duration, ntargs) 
            q_targets = [
            ...
            actions_out = [action.clone() for _ in range(ntargs)]
            # print("Action Out Shape", len(actions_out))
            self._planner_call_count += 1
            print("\n------------------------------ COUNT{self._planner_call_count} ------------------------------\n" )
            for i in range(ntargs):
                q_targ = q_targets[i]
                # print(q_targ.values())
                # print(q_targ.values())
                # print("Action Out Keys", q_targ.keys())
                for grp in ctrl_groups:
                    ...
                    # (Pdb) self.controller_info['trunk']
                    # {'start_idx': 3, 'dofs': array([6, 7, 8, 9]), 'command_dim': 4}
                    # (Pdb) self.controller_info['arm_left']
                    # {'start_idx': 7, 'dofs': array([10, 12, 14, 16, 18, 20, 22]), 'command_dim': 7}
                    # (Pdb) self.controller_info['arm_right']
                    # {'start_idx': 15, 'dofs': array([11, 13, 15, 17, 19, 21, 23]), 'command_dim': 7}
                    start = self.controller_info[grp]["start_idx"]
            ...
            import traceback as tb
            tb.print_exc()
        # print("Action_out len", len(actions_out))
        # print(actions_out)
        # print()
        self._last_solve_t = now
        self._target_dirty = False
        # Update last desired pose after solving IK
        for arm in self.robot.arm_names:
        ...
        return actions_out, self.planner, q_targets

        # print("[Test 13] Getting EEF pose after IK")
        # cur_pos, cur_quat = self._get_eef_pose(arm)
        # print(
        #     f"[EEF after IK] {arm}: pos={cur_pos.tolist()}, quat(xyzw)={cur_quat.tolist()}"
        # )
        # self._head_nudge = 0.0
        # self._head_yaw_nudge = 0.0
        # self._last_solve_t = now
        ...
        #         self.desired[arm][1].clone()
        #     )
        # # Note: action is actually a list of actions for the trajectory
        # return action
...
def check_pose_reached(teleop, pos_threshold: float = 0.02, ori_threshold: float = 0.05) -> bool:
    """Check if all arms have reached their desired poses.

    Args:
        teleop: EEFBaseTeleop instance
    ...
    for arm_name in teleop.robot.arm_names:
        ...
        cur_pos, cur_quat = teleop._get_eef_pose(arm_name)

        # Get desired pose
        des_pos, des_quat = teleop.desired[arm_name]
...
def main():
    ...
    try:
        while True:
            for _ in range(5):
                env.step(th.zeros(23))
            # goal_val_x = float(input("====================== Enter X: "))
            # goal_val_y = float(input("====================== Enter Y: "))
            # goal_val_rz = float(input("====================== Enter Rz: "))
            import numpy as np
            np.set_printoptions(precision=9, suppress=True, linewidth=1000)
            # val_speed = np.mean([1/1.4568, 0.75/1.077796432, 0.5/0.694067849])
            # val_rad = np.mean([15/14.13991705579623, 15/14.132091556489378, 10/7.805180588223393])
            #duration 15, timestep 30
            # goal_x = np.array([0.05, 0.1, 0.15, 0.125, 0.175, 0.14, 0.25, 0.23, 0.4])
            # true_x = np.array([0.020022475, 0.073144917, 0.125177536, 0.098410601, 0.15022953, 0.114752548, 0.229525112, 0.208493589, 0.38602397])
            
            #duration 10, timestep30
            # goal_x = np.array([0.05, 0.1, 0.13, 0.2, 0.18, 0.07, 0.11, 0.16, 0.14, 0.02, 0.03, 0.01,0.06, 0.25])
            # true_x = np.array([0.053290991, 
            #                     0.127586768, 
            #                     0.1722985, 
            #                     0.276871142, 
            #                     0.246822184, 
            ...
            # goal_rz = np.array([10, 15, 20, 25, 1, 3, 6, 8, 9, 12, 17])
            # true_rz = np.array([27.846629279522165, 
            #                     43.43927938425453, 
            #                     59.0390364457757, 
            #                     74.37344046190078,
            #                     0.3059897168533261,
            ...
            #     sign = np.sign(desired_x)
            #     desired_x = np.abs(desired_x)
            #     a2, a1, a0 = 0.4406, 1.1429, -0.0082  # from refit
            #     return sign * (-a1 + np.sqrt(a1**2 - 4*a2*(a0 - desired_x))) / (2 * a2)

            # def command_from_desired(desired_x):
            #     sign = np.sign(desired_x)
            #     desired_x = np.abs(desired_x)
            #     a3, a2, a1, a0 = 1.0144, 0.1577, 1.1192, -0.0067 # Batch 1
            #     # a3, a2, a1, a0 = 0.6676, 0.3693, 1.0779, -0.0046 # Batch 2
            #     a4, a3, a2, a1, a0 = -0.3055, 1.6744, -0.0562, 1.0607, -0.0053
            #     """Compute the command distance that should yield desired_x (m)."""
            #     # coefficients for cubic equation: a3*x^3 + a2*x^2 + a1*x + (a0 - desired_x) = 0
            #     coeffs = [a4, a3, a2, a1, a0 - desired_x]
            #     roots = np.roots(coeffs)
            #     # select the real positive root (the physically meaningful one)
            #     real_roots = roots[np.isreal(roots)].real
            #     valid_roots = real_roots[(real_roots > 0)]
            #     if len(valid_roots) == 0:
            #         return np.nan
            #     return sign * valid_roots.min()  # smallest positive root is the right branch

            from b1k.planner.full_planner import FullBodyPlanner

            planner = FullBodyPlanner()

            qc_joint_names = []
            for i in range(1, 5):
                qc_joint_names.append(f"torso_joint{i}")
            for i in range(1, 8):
                qc_joint_names.append(f"left_arm_joint{i}")
            qc_joint_names.append("left_gripper_width")
            for i in range(1, 8):
                qc_joint_names.append(f"right_arm_joint{i}")
            qc_joint_names.append("right_gripper_width")

            def get_current_joint_state(joint_names):
                ...
                for n in joint_names:
                    if n in index:
                        ...
                        q_dict[n] = 0.0
                # return [q_dict[n] for n in joint_names]
                return q_dict

            qc = get_current_joint_state(qc_joint_names)
            duration = 10.0
            pG_left = [1.82, 0.1, 0.75]
            pG_right = [1.8, -0.4, 0.8]

            # === T=0: Record initial robot base frame ===
            from scipy.spatial.transform import Rotation as R
            pos_initial = robot.get_position_orientation()[0]
            quat_initial = robot.get_position_orientation()[1]
            pos_initial_np = pos_initial.detach().cpu().numpy()
            quat_initial_np = quat_initial.detach().cpu().numpy()
            rot_initial = R.from_quat(quat_initial_np)

            # Build initial transformation matrix (robot base at T=0)
            T_initial = np.eye(4)
            T_initial[:3, :3] = rot_initial.as_matrix()
            T_initial[:3, 3] = pos_initial_np

            print("\n=== T=0: Initial Robot Base Frame ===")
            print(f"Position: {pos_initial_np}")
            print(f"Quaternion [x,y,z,w]: {quat_initial_np}")
            print(f"Euler (roll, pitch, yaw): {rot_initial.as_euler('xyz', degrees=True)}")

            print("\n=== Goal End-Effector Positions (given to planner) ===")
            print(f"Left Goal Position: {pG_left}")
            print(f"Right Goal Position: {pG_right}")

            T = planner.whole_body.chain.forward_kinematics(qc)

            from scipy.spatial.transform import Rotation as Rot

            rot_offset = Rot.from_euler("y", 30, degrees=True)  # x,y,z,w
            rG_left = (rot_offset * Rot.from_quat(T['left_gripper_link'].rotation().as_quat().toarray().flatten())).as_quat().tolist()
            rG_right = (rot_offset * Rot.from_quat(T['right_gripper_link'].rotation().as_quat().toarray().flatten())).as_quat().tolist()

            qc_= [qc[n] for n in qc_joint_names]
            actions = planner.move_base(duration, pG_left, rG_left, pG_right, rG_right, qc_)

            print("actions length", type(actions), len(actions))

            np.set_printoptions(precision=3, suppress=True, linewidth=1000)

            start = robot.get_joint_positions()[:6].detach().cpu().numpy()
            print("\n=== Executing Actions ===")
            print("Start joint positions:", start)

            # Execute all actions
            for act in actions:
                env.step(act)

            finish = robot.get_joint_positions()[:6].detach().cpu().numpy()
            print("Finish joint positions:", finish)
            print("Joint Diff:", finish - start)

            # === T=t: Get final end-effector positions and transform back to T=0 frame ===

            # Get current robot base position and orientation at T=t
            pos_current = robot.get_position_orientation()[0]
            quat_current = robot.get_position_orientation()[1]
            pos_current_np = pos_current.detach().cpu().numpy()
            quat_current_np = quat_current.detach().cpu().numpy()
            rot_current = R.from_quat(quat_current_np)

            # Build current robot base transformation matrix (at T=t)
            T_current = np.eye(4)
            T_current[:3, :3] = rot_current.as_matrix()
            T_current[:3, 3] = pos_current_np

            print("\n=== T=t: Current Robot Base Frame ===")
            print(f"Position: {pos_current_np}")
            print(f"Quaternion [x,y,z,w]: {quat_current_np}")
            print(f"Euler (roll, pitch, yaw): {rot_current.as_euler('xyz', degrees=True)}")

            # Get final end-effector positions (relative to CURRENT robot base at T=t)
            pos_final_left_in_current_base, quat_final_left = teleop._get_eef_pose("left")
            pos_final_right_in_current_base, quat_final_right = teleop._get_eef_pose("right")

            pos_final_left_in_current = pos_final_left_in_current_base.detach().cpu().numpy()
            pos_final_right_in_current = pos_final_right_in_current_base.detach().cpu().numpy()

            print("\n=== T=t: Final End-Effector Positions (in current robot base frame) ===")
            print(f"Left End-Effector: {pos_final_left_in_current}")
            print(f"Right End-Effector: {pos_final_right_in_current}")

            # Build transformation matrices for end-effectors in current robot frame
            T_ee_in_current_left = np.eye(4)
            T_ee_in_current_left[:3, 3] = pos_final_left_in_current

            T_ee_in_current_right = np.eye(4)
            T_ee_in_current_right[:3, 3] = pos_final_right_in_current

            # Transform to world frame: T_ee_world = T_current @ T_ee_in_current
            T_ee_world_left = T_current @ T_ee_in_current_left
            T_ee_world_right = T_current @ T_ee_in_current_right

            # Transform from world frame back to initial robot frame (T=0):
            # T_ee_in_initial = T_initial^-1 @ T_ee_world
            T_ee_in_initial_left = np.linalg.inv(T_initial) @ T_ee_world_left
            T_ee_in_initial_right = np.linalg.inv(T_initial) @ T_ee_world_right

            # Extract positions in initial robot frame (T=0)
            pos_final_left_in_initial_frame = T_ee_in_initial_left[:3, 3]
            pos_final_right_in_initial_frame = T_ee_in_initial_right[:3, 3]

            print("\n=== Final End-Effector Positions (transformed to T=0 initial robot frame) ===")
            print(f"Left End-Effector: {pos_final_left_in_initial_frame}")
            print(f"Right End-Effector: {pos_final_right_in_initial_frame}")

            print("\n=== Comparison: Goal vs Reached (both in T=0 initial robot frame) ===")
            print(f"Left Goal:    {np.array(pG_left)}")
            print(f"Left Reached: {pos_final_left_in_initial_frame}")
            print(f"Left Error:   {pos_final_left_in_initial_frame - np.array(pG_left)}")
            print(f"Left Error Magnitude: {np.linalg.norm(pos_final_left_in_initial_frame - np.array(pG_left)):.6f}")

            print(f"\nRight Goal:    {np.array(pG_right)}")
            print(f"Right Reached: {pos_final_right_in_initial_frame}")
            print(f"Right Error:   {pos_final_right_in_initial_frame - np.array(pG_right)}")
            print(f"Right Error Magnitude: {np.linalg.norm(pos_final_right_in_initial_frame - np.array(pG_right)):.6f}")

            raise ValueError("STOP HERE")


            goal = [
                corrected_command(goal_val_x), 
                corrected_command(goal_val_y), 
                radians(corrected_command_z(goal_val_rz))
            ]
            ...
            teleop.nav_plan.reset(config)
            
            if teleop.nav_plan.solve():
                teleop.nav_plan.plot()
                print("Nav planner solved")
            else:
                raise RuntimeError("failed to solve")

            # get_nav_control = teleop.nav_plan.get_omni_gibson_control()
            # lin_vel, ang_vel = teleop.nav_plan.get_solution()
            if True:
            # if teleop.nav_plan.control_mode == 'velocity':
                # x_, y_, rz_ = teleop.nav_plan.get_solution()
                x_, y_, rz_ =  teleop.nav_plan.get_solution()
            else:
                sol = teleop.nav_plan.get_solution()
            ...
            # if teleop.nav_plan.control_mode == 'velocity':
                ...
                posb, quatb = robot.get_position_orientation(frame="world")
                posbb, quatbb = robot.get_position_orientation(frame="world")
                print("POS", posb)
                print("Quat", quatb)
                while t < config['duration']:
                    act = np.zeros((23,), dtype=float)
                    act[0] =  x_(t)
                    act[1] =  y_(t)
                    act[2] =  rz_(t)
                    # print(act)
                    ...
                    # print("POS diff", th.linalg.norm(pos - posb))
                    # # Compute relative rotation and convert to Euler angles
                    # from scipy.spatial.transform import Rotation as R
                    # quat_np = quat.cpu().numpy()  # Convert to numpy: [x, y, z, w]
                    # quatb_np = quatb.cpu().numpy()
                    # # scipy expects [x, y, z, w] format (same as torch)
                    # rot_current = R.from_quat(quat_np)
                    # rot_previous = R.from_quat(quatb_np)
                    # rot_diff = rot_previous.inv() * rot_current
                    # euler_diff = rot_diff.as_euler('xyz', degrees=True)  # roll, pitch, yaw in degrees
                    # print(f"Quat diff (Euler): roll={euler_diff[0]:.2f}°, pitch={euler_diff[1]:.2f}°, yaw={euler_diff[2]:.2f}°")
                    # posb = pos.clone()
                    # quatb = quat.clone()


                pos, quat = robot.get_position_orientation(frame="world")
                print("POS diff NORM", th.linalg.norm(pos - posbb))
                print("POS diff FINAL", (pos - posbb))
                # Compute relative rotation and convert to Euler angles
                from scipy.spatial.transform import Rotation as R
                import numpy as np
                quat_np = quat.cpu().numpy()  # Convert to numpy: [x, y, z, w]
                quatb_np = quatbb.cpu().numpy()
                # scipy expects [x, y, z, w] format (same as torch)
                rot_current = R.from_quat(quat_np)
                rot_previous = R.from_quat(quatb_np)
                rot_diff = rot_previous.inv() * rot_current
                euler_diff = rot_diff.as_euler('zyx', degrees=True)[0] # roll, pitch, yaw in degrees
                print(f"Final RZ",euler_diff)

                # Calculate full relative transformation (end frame w.r.t. initial frame)
                pos_initial_np = posbb.cpu().numpy()
                pos_final_np = pos.cpu().numpy()

                # Create 4x4 transformation matrices
                T_initial = np.eye(4)
                T_initial[:3, :3] = rot_previous.as_matrix()
                T_initial[:3, 3] = pos_initial_np

                T_final = np.eye(4)
                T_final[:3, :3] = rot_current.as_matrix()
                T_final[:3, 3] = pos_final_np

                # Calculate relative transformation: T_relative = T_initial^-1 * T_final
                T_relative = np.linalg.inv(T_initial) @ T_final

                # Extract relative position and orientation
                pos_relative = T_relative[:3, 3]
                rot_relative_matrix = T_relative[:3, :3]
                rot_relative = R.from_matrix(rot_relative_matrix)
                quat_relative = rot_relative.as_quat()
                euler_relative = rot_relative.as_euler('xyz', degrees=True)

                print("\n=== Relative Transformation (End w.r.t. Initial Frame) ===")
                print(f"Relative Position (in initial frame): {pos_relative}")
                print(f"Relative Quaternion [x,y,z,w]: {quat_relative}")
                print(f"Relative Euler (roll, pitch, yaw): [{euler_relative[0]:.2f}°, {euler_relative[1]:.2f}°, {euler_relative[2]:.2f}°]")
                print(f"Relative Transformation Matrix:\n{T_relative}")
                
            else:

                for i in range(teleop.nav_plan.T-1):
                # for i in range(99999):
                    # input()
                    act = np.zeros((23,), dtype=float)
                    act[0] = 0.25#sol['dx_b'][i]
                    act[1] = 0#0.25#sol['dy_b'][i]
                    act[2] = 0.25#sol['dtheta'][i]
                    # print(act)
                    env.step(act)
                    pos, quat = robot.get_position_orientation(frame="world")
                    print("POS diff", th.linalg.norm(pos - posb))
