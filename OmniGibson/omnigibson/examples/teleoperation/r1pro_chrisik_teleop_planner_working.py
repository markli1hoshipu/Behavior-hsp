...
class EEFBaseTeleop:
    ...
    def step(self) -> th.Tensor:
        ...
        if (
            self._drive_active
            and time_since_last >= self._ik_min_period
            and not self._target_dirty
        ):
            self._target_dirty = True

        need_ik = self._target_dirty and time_since_last >= self._ik_min_period
        print(
            f"[Test 8] Checking IK: need_ik={need_ik}, target_dirty={self._target_dirty}"
        )
        ...
        print("[Test 8.0] IK needed - preparing to solve")
        print(f"  Desired arms: {list(self.desired.keys())}")
        print(
            f"  Left arm target: pos={self.desired['left'][0].tolist()}, quat={self.desired['left'][1].tolist()}"
        )
        print(
            f"  Right arm target: pos={self.desired['right'][0].tolist()}, quat={self.desired['right'][1].tolist()}"
        )
        ...
        ntargs= 90# increasse number to slow robot
        ...
        # try:
        #     targets = self.ik.solve_to_joint_targets(
        #         self.desired["left"],
        #         self.desired["right"],
        #     )
        # except Exception as e:
        ...
        # print("[Test 9] IK solve completed")

        # if targets is None:
        #     print("[Test 9.1] IK returned None, using zero action")
        #     return action

        # >>TODO: need to remap q_targets to a format required for the controller (delta)<< 

        print("[Test 10] Building joint mappings")
        name_to_joint_index = {
            name: i for i, name in enumerate(self.robot.joints.keys())
        }

        print("[Test 11] Applying IK targets to action")
        ...
        actions_out = [action]*ntargs
        print("Action Out Shape", len(actions_out))

        for i in range(ntargs):
            
            q_targ = q_targets[i]
            print("Action Out Shape", q_targ.keys)
            ...
            for grp in ctrl_groups:
                ...
                for local_i, j_idx in enumerate(dof_idx.tolist()):
                    ...
                    if j_name in q_targ:
                        ...
                        q_ = float(q_targ[j_name])
                        if grp == "trunk":
                            actions_out[i][start + local_i] += q_
                        else:
                            actions_out[i][start + local_i] = q_
        ...
        # print("[Test 12] Applying head nudges")
        # if pitch_idx is not None:
        #     action[pitch_idx] += pitch_val
        # if yaw_idx is not None:
        #     action[yaw_idx] += yaw_val
...
def initialize_robot_to_init_state(robot, verbose: bool = True) -> None:
    """Initialize robot joints to r1pro_init_joint_state configuration."""
    try:
        ...
        for joint_name, target_position in r1pro_init_joint_state.items():
        ...
        if indices:
            idx_tensor = th.tensor(indices, dtype=th.long, device=device)
            pos_tensor = th.tensor(targets, dtype=dtype, device=device)
            robot.set_joint_positions(
                positions=pos_tensor, indices=idx_tensor, drive=False
            )
            if verbose:
                print(f"[Teleop] Initialized {len(indices)} joints to r1pro_init_joint_state.")
    except Exception as e:
        if verbose:
            print(f"[Teleop] Failed to initialize robot to init state: {e}")
...
def main():
    ...
    env.reset()
    # Initialize robot to r1pro_init_joint_state
    initialize_robot_to_init_state(robot)
    ...
    def reload_instance():
        ...
        initialize_robot_to_init_state(robot, verbose=False)
        ...
        while True:
            try:
                ...
                if type(action) == list:
                    for act in action:
                        print(
                        f"[Test 2] Action computed: shape={act.shape}, norm={act.norm():.4f}"
                        )
                        env.step(act)
                else:
                    env.step(action)
