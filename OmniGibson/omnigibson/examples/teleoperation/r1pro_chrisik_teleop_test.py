...
class EEFBaseTeleop:
    ...
    def step(self) -> th.Tensor:
        ...
        try:
            ...
            print(f"[Test 8] Checking IK: need_ik={need_ik}, target_dirty={self._target_dirty}")
            ...
            print(f"  Left arm target: pos={self.desired['left'][0].tolist()}, quat={self.desired['left'][1].tolist()}")
            print(f"  Right arm target: pos={self.desired['right'][0].tolist()}, quat={self.desired['right'][1].tolist()}")
...
def main():
    ...
    try:
        while True:
            ...
            action = teleop.step()
            print(f"[Test 2] Action computed: shape={action.shape}, norm={action.norm():.4f}")
            env.step(action)
    finally:
        og.shutdown()
