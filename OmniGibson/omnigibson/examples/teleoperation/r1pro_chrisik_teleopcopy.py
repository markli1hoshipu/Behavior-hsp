...
class EEFBaseTeleop:
    ...
    def __init__(
        ...
        from b1k.nav_plan import NavPlanner

        self.nav_plan = NavPlanner(60)
    ...
    def step(self) -> th.Tensor:
        ...
        if self._pending_sam6d_pose_base is not None:
            ...
            try:
                ...
                quat_target = th.tensor(Rot.from_euler("xyz", [0, np.pi*-0.5, 0.0]).as_quat())
                quat_target = th.tensor([0,1,0,0])
                pos_target = th.tensor([0.45, 0.45,  0.95])
                pos_target_r = th.tensor([0.7, -0.45,  0.4])
        ...
        try:
            ...
            ntargs = 160#75000# increasse number to slow robot
                ...
                for grp in ctrl_groups:
                    ...
                    for local_i, j_idx in enumerate(dof_idx.tolist()):
                        ...
                        if j_name in q_targ:
                            ...
                            # because command_output_limits="default" expects normalized input
                            normalized_cmd = normalize_joint_position(
                                self.robot, j_name, float(q_targ[j_name])
                            )
...
def normalize_joint_position(robot, joint_name: str, position: float) -> float:
    ...
    joint = robot.joints[joint_name]
    if not (hasattr(joint, 'lower_limit') and hasattr(joint, 'upper_limit')):
        return 0.0
    ...
    # normalized = 2 * (position - lower) / (upper - lower) - 1
...
def print_joint_limits(robot):
    ...
    for i, joint_name in enumerate(joint_names):
        ...
        if hasattr(joint, 'lower_limit') and hasattr(joint, 'upper_limit'):
            lower = float(joint.lower_limit) if joint.lower_limit is not None else "None"
            upper = float(joint.upper_limit) if joint.upper_limit is not None else "None"
            print(f"  Joint {i:2d} ({joint_name:30s}): lower={lower:+8.4f}, upper={upper:+8.4f}" if isinstance(lower, float) else f"  Joint {i:2d} ({joint_name:30s}): lower={lower}, upper={upper}")
        else:
            print(f"  Joint {i:2d} ({joint_name:30s}): No limit attributes")

    # Also check controller limits
    if hasattr(robot, '_controllers'):
        print("\n[Controller Output Limits]:")
        for controller_name, controller in robot._controllers.items():
            if hasattr(controller, 'command_output_limits'):
                print(f"  {controller_name}: {controller.command_output_limits}")


def check_joints_reached(robot, target_joints: th.Tensor, joint_threshold: float = 0.01) -> bool:
    ...
    try:
        ...
        # ro = fk.rotation().as_quat().toarray().flatten() # numpy.ndarray (4,)
            ...
            if type(action) == tuple:
                ...
                target_joints_tensor = teleop.robot.get_joint_positions().clone()
                for joint_name, joint_value in target_joints_dict.items():
                    joint_idx = joint_names.index(joint_name)
                    target_joints_tensor[joint_idx] = float(joint_value)
                check_joints_reached(teleop.robot, target_joints_tensor)
                # print_joint_limits(teleop.robot)
