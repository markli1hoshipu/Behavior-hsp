    ...
    def step(self) -> th.Tensor:
        ...
        if self._pending_sam6d_pose_base is not None:
            ...
            try:
                ...
                pos_target = th.tensor([0.4, 0.25,  0.9])
                pos_target_r = th.tensor([0.4, -0.25,  0.9])
                ...
                for grp in ctrl_groups:
                    ...
                    for local_i, j_idx in enumerate(dof_idx.tolist()):
                        ...
                        if j_name in q_targ:
                            ...
                            dq = float(q_targ[j_name]) - float(q[j_idx])
                            # Trunk writes should be additive to allow manual head pitch/yaw overlays
                            if grp == "trunk":
                                actions_out[i][start + local_i] += dq
                            else:
                                actions_out[i][start + local_i] = dq
                # Update q to the new target for next iteration (incremental trajectory)
                for j_name, j_val in q_targ.items():
                    j_idx = list(self.robot.joints.keys()).index(j_name)
                    q[j_idx] = float(j_val)
...
def check_joints_reached(robot, target_joints: th.Tensor, joint_threshold: float = 0.01) -> bool:
    ...
    for i in range(len(current_joints)):
        ...
        if not joint_ok:
            ...
            print(f"  Joint {i}: current={current_joints_np[i]:+.4f}, "
                  f"target={target_joints_np[i]:+.4f}, "
                  f"error={joint_errors_np[i]:.4f}rad (ok={joint_ok})")

    # Print summary statistics
    max_error_idx = th.argmax(joint_errors).item()
    print(f"  Max error: {joint_errors_np[max_error_idx]:.4f}rad at joint {max_error_idx}")
