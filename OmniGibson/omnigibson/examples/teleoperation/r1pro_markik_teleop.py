...
class GraspSolverSocketClient:
    ...
    def _receive_json(self, sock: socket.socket) -> Dict[str, object]:
        ...
        if length_bytes is None:
            raise ConnectionError("SAM-6D server closed connection before sending response length.")
        msg_len = struct.unpack("!I", length_bytes)[0]
        payload = self._recv_exact(sock, msg_len)
        if payload is None:
            raise ConnectionError("SAM-6D server closed connection before sending response payload.")
...
class EEFBaseTeleop:
    ...
    def __init__(
        ...
        self._sam6d_target_label = sam6d_target_label.lower() if sam6d_target_label else None
        ...
        self._pending_sam6d_pose_base: Optional[Tuple[str, th.Tensor, th.Tensor, str]] = None
        ...
        self._pending_contact_pose_base: Optional[Tuple[str, th.Tensor, th.Tensor, str]] = None
        ...
        self._grasp_viz_pose: Dict[str, Optional[Tuple[th.Tensor, th.Tensor]]] = {arm: None for arm in robot.arm_names}
    ...
    def _register_keys(self):
        ...
        KeyboardEventHandler.add_keyboard_callback(
            lazy.carb.input.KeyboardInput.W, lambda: self._nudge_base(+self.base_lin_step, 0.0, 0.0)
        )
        KeyboardEventHandler.add_keyboard_callback(
            lazy.carb.input.KeyboardInput.S, lambda: self._nudge_base(-self.base_lin_step, 0.0, 0.0)
        )
        KeyboardEventHandler.add_keyboard_callback(
            lazy.carb.input.KeyboardInput.A, lambda: self._nudge_base(0.0, +self.base_lin_step, 0.0)
        )
        KeyboardEventHandler.add_keyboard_callback(
            lazy.carb.input.KeyboardInput.D, lambda: self._nudge_base(0.0, -self.base_lin_step, 0.0)
        )
        KeyboardEventHandler.add_keyboard_callback(
            lazy.carb.input.KeyboardInput.Q, lambda: self._nudge_base(0.0, 0.0, +self.base_yaw_step)
        )
        KeyboardEventHandler.add_keyboard_callback(
            lazy.carb.input.KeyboardInput.E, lambda: self._nudge_base(0.0, 0.0, -self.base_yaw_step)
        )
        # Head tilt (joint-based, pitch)
        KeyboardEventHandler.add_keyboard_callback(
            lazy.carb.input.KeyboardInput.PAGE_UP, lambda: self._tilt_head(+float(self.head_pitch_step))
        )
        KeyboardEventHandler.add_keyboard_callback(
            lazy.carb.input.KeyboardInput.PAGE_DOWN, lambda: self._tilt_head(-float(self.head_pitch_step))
        )
        # Head turn (joint-based, yaw) left / right
        # Use J (left) and L (right) to avoid overlap with EEF keys
        KeyboardEventHandler.add_keyboard_callback(
            lazy.carb.input.KeyboardInput.J, lambda: self._yaw_head(+float(self.head_yaw_step))
        )
        KeyboardEventHandler.add_keyboard_callback(
            lazy.carb.input.KeyboardInput.L, lambda: self._yaw_head(-float(self.head_yaw_step))
        )
    ...
    def _start_sam6d_request(self):
        ...
        self._sam6d_last_cam_pose = (cam_pos_np.copy(), cam_quat_np.copy(), self.active_arm)
        print(self._sam6d_last_cam_pose, "markhoshipuuuuuuuuuuuuuuuuuuuu")
        ...
        if self._sam6d_target_label:
            ...
            if label_key in lookup_lower:
                ...
                print(f"[SAM-6D] Unknown semantic label '{self._sam6d_target_label}'. Skipping mask generation.")
                self._sam6d_warned_label = True
        # if mask is None:
        #     # Fallback: choose most common semantic ID (excluding background) as target
        if mask is None:
            print(
        ...
        print(f"[SAM-6D] Sending capture to {host}:{port} (H={rgb.shape[0]}, W={rgb.shape[1]}).")
    ...
    def _handle_sam6d_response(self, message: Dict[str, object]):
        ...
        if not isinstance(message, dict):
            ...
            return
        #print(f"[SAM-6D] Received response: {message}")
        #pdb.set_trace()
        ...
        if pose_dict and isinstance(pose_dict, dict):
            ...
            trans = trans or pose_dict.get("grasp_translation_m") or pose_dict.get("position")
            quat = quat or pose_dict.get("quaternion")
        if rot is not None:
            R = np.asarray(rot, dtype=np.float32)
            if R.shape != (3, 3):
                self._pending_sam6d_error = f"SAM-6D rotation must be 3x3. Got shape {R.shape}."
                return
            ...
            if quat_np.size != 4:
                ...
                return
            R = T.quat2mat(th.tensor(quat_np, dtype=th.float32)).detach().cpu().numpy().astype(np.float32)
        else:
        ...
        if t.size != 3:
            self._pending_sam6d_error = f"SAM-6D translation must have 3 elements. Got {t}."
            return
        ...
        print(f"[SAM-6D] Received pose from server for arm '{arm}': R=\n{R}\nt={t.tolist()}")
        print(f"[camera pose] pos={cam_pose_np[0].tolist() if cam_pose_np is not None else None}")
        try:
            ...
            print(f"[SAM-6D] Base pose ({arm}) pos={pos_base.tolist()} quat={quat_base.tolist()}")
            self._pending_sam6d_pose_base = (arm, pos_base.detach().clone(), quat_base.detach().clone(), "sam6d")
            ...
            print(f"[SAM-6D] Deferred base-frame conversion ({exc}); will convert in main loop.")
        finally:
    ...
    def _start_contact_request(self):
        ...
        if mask is None or not np.any(mask):
            print("[Contact] No target mask available (target label / object unresolved or absent); cancelling request.")
            return
        ...
        print(f"[Contact] Sending capture to {host}:{port} (H={rgb.shape[0]}, W={rgb.shape[1]}).")
    ...
    def _handle_contact_response(self, message: Dict[str, object]):
        ...
        if arm not in self.desired:
            self._pending_contact_error = f"Contact-GraspNet response for unknown arm '{arm}'."
            return
        ...
        for idx, pose in enumerate(poses[: self._contact_max_candidates]):
            ...
            quat = np.asarray(
                pose.get("quaternion") or pose.get("quat") or pose.get("q"), dtype=np.float32
            ).reshape(-1)
        ...
        if not queue:
            self._pending_contact_error = "Contact-GraspNet returned no valid pose entries."
            return
...
def main():
    ...
    parser.add_argument(
        "--sam6d-timeout",
        type=float,
        default=_env_cast("SAM6D_TIMEOUT", float, 30.0),
    ...
    if grasp_provider == "sam6d":
        if args.sam6d_disable:
            print("[Teleop] SAM-6D provider selected but SAM-6D client disabled; grasp capture will be unavailable.")
        elif sam6d_host:
            ...
            print("[Teleop] SAM-6D provider selected but no SAM-6D host specified; grasp capture will be unavailable.")
    elif grasp_provider == "contact":
        if args.contact_disable:
            print("[Teleop] Contact-GraspNet provider selected but contact client disabled; grasp capture will be unavailable.")
        elif contact_host:
            ...
            print("[Teleop] Contact-GraspNet provider selected but no contact host specified; grasp capture will be unavailable.")
    sam6d_target_label = args.sam6d_target_label.strip() if isinstance(args.sam6d_target_label, str) else None
    sam6d_target_object = args.sam6d_target_object.strip() if isinstance(args.sam6d_target_object, str) else None
    ...
    if grasp_provider == "sam6d" and teleop.sam6d_client is not None:
        print("H: capture head RGB/depth/semantic, send to SAM-6D, auto-target when pose arrives")
