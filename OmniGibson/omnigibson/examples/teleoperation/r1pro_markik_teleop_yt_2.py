...
Notes
- Controls one arm at a time; that arm drives its own joints plus the torso (whole-body IK).
- Reference frame is the robot base; key nudges apply in base coordinates, not the EEF frame.
- Requires a solver available at ~/mark_ik (or set MARK_IK_PATH) exposing a WholeBodyIK-like API.
  The adapter tries common entry points. If it fails to import, it will print a helpful message.
"""
...
from omnigibson.utils.constants import semantic_class_id_to_name, semantic_class_name_to_id
from omnigibson.utils.python_utils import recursively_convert_to_torch
from omnigibson.utils.ui_utils import KeyboardEventHandler, draw_line, clear_debug_drawing
...
def apply_task_instance_state(env: og.Environment, instance_id: int) -> None:
    ...
    tro_path = Path(root) / "json" / f"{scene_model}_task_{env.task.activity_name}_instances" / f"{template_name}-tro_state.json"
...
class MarkIKAdapter:
    """Adapter for /home/aaron/mark_ik solver (hsp_r1pro_iksolver.solve_ik).

    - Builds mode="left_torso" or "right_torso"
    - Sends base-frame EEF target (pos[m], quat[xyzw]) and current joint guess
    - Returns a dict: {joint_name: absolute_position}
    """

    def __init__(self, robot):
        self.robot = robot
        # Prefer the installed package entry point
        try:
            from mark_ik import solve_ik  # type: ignore
        except Exception:
            # Fallback to module path inside the package
            try:
                from mark_ik.hsp_r1pro_iksolver import solve_ik  # type: ignore
            except Exception as e:
                solve_ik = None  # type: ignore
                print(f"[MarkIKAdapter] Could not import solve_ik from installed mark_ik package: {e}")
    ...
    def solve_to_joint_targets(self, arm: str, target_pose_base: Tuple[th.Tensor, th.Tensor]) -> Optional[Dict[str, float]]:
        ...
        # Optional URDF path: honor env var if provided
        #urdf_path = os.environ.get("R1PRO_URDF", None)
        urdf_path = '/home/user/BEHAVIOR-1K/OmniGibson/omnigibson/examples/teleoperation/mark_ik/src/mark_ik/r1pro.urdf'
        # Initial guess
    ...
    def solve_full(self, arm: str, target_pose_base: Tuple[th.Tensor, th.Tensor]) -> Optional[Dict[str, object]]:
        """Call the solver and return its full result dict, plus joint_names ordering."""
        if self.solve_ik is None:
            return None
        pos_t, quat_t = target_pose_base
        pos = pos_t.detach().cpu().numpy().astype("float64")
        ...
        mode = "left_torso" if arm == "left" else "right_torso"
        urdf_path = '/home/aaron/mark_ik/src/mark_ik/r1pro.urdf'
        #urdf_path = os.environ.get("R1PRO_URDF", None)
        q0 = self._initial_guess(arm)
        try:
            result = self.solve_ik(
            ...
            if isinstance(result, dict):
                result["joint_names"] = self._desired_joint_names(arm)
            return result
        except Exception as e:
            print(f"[MarkIKAdapter] solve_full failed: {e}")
            return None
...
class XVLAHttpClient:
    ...
    def act(
        ...
        r = requests.post(self.server_url + "/act", json=payload, timeout=self.timeout)
        r.raise_for_status()
        out = r.json()
        if "action" not in out:
            raise KeyError(f"XVLA response missing 'action'. Keys={list(out.keys())}")
...
class EEFBaseTeleop:
    ...
    def __init__(
        ...
        env,
        action_jsonl_path,
        xvla_client=None, xvla_instruction="", xvla_domain_id=0, xvla_steps=10
    ):
        self.robot = robot
        self.env = env
        
        self.xvla_client = xvla_client
        self.xvla_mode = False
        self._xvla_instruction = xvla_instruction
        self._xvla_domain_id = int(xvla_domain_id)
        self._xvla_steps = int(xvla_steps)

        self._xvla_capture_pending = False
        self._xvla_inflight = False
        self._xvla_error: Optional[str] = None
        self._xvla_action_queue = deque()   # each element: th.Tensor(action_dim,)
        
        self.active_arm = "left"
        # desired poses per arm, initialized to current EEF pose
        self.desired: Dict[str, Tuple[th.Tensor, th.Tensor]] = {}
        self._action_f = open(action_jsonl_path, "w", buffering=1)
        self._control_key_pressed = False
        ...
        # step sizes
        self.pos_step = 0.025  # meters per repeat (slightly larger to reduce required IK steps)
        ...
        self._head_binding: Optional[Tuple[str, int, str]] = None  # pitch: (component, action_index, joint_name)
        ...
        self._eef_rot_correction = th.tensor([[0.0, -1.0, 0.0],
                                              [1.0,  0.0, 0.0],
    ...
    def _mark_control_key(self):
        self._control_key_pressed = True
    ...
    def _register_keys(self):
        ...
        KeyboardEventHandler.add_keyboard_callback(lazy.carb.input.KeyboardInput.UP, lambda: (self._mark_control_key(), self._bump_xyz(+self.pos_step, 0, 0)))
        KeyboardEventHandler.add_keyboard_callback(lazy.carb.input.KeyboardInput.DOWN, lambda: (self._mark_control_key(), self._bump_xyz(-self.pos_step, 0, 0)))
        KeyboardEventHandler.add_keyboard_callback(lazy.carb.input.KeyboardInput.LEFT, lambda: (self._mark_control_key(), self._bump_xyz(0, +self.pos_step, 0)))
        KeyboardEventHandler.add_keyboard_callback(lazy.carb.input.KeyboardInput.RIGHT, lambda: (self._mark_control_key(), self._bump_xyz(0, -self.pos_step, 0)))
        KeyboardEventHandler.add_keyboard_callback(lazy.carb.input.KeyboardInput.P, lambda: (self._mark_control_key(), self._bump_xyz(0, 0, +self.pos_step)))
        KeyboardEventHandler.add_keyboard_callback(lazy.carb.input.KeyboardInput.SEMICOLON, lambda: (self._mark_control_key(), self._bump_xyz(0, 0, -self.pos_step)))

        # Orientation nudges (base frame axes: x,y,z)
        KeyboardEventHandler.add_keyboard_callback(lazy.carb.input.KeyboardInput.N, lambda: (self._mark_control_key(), self._bump_rpy(+self.rot_step, 0, 0)))
        KeyboardEventHandler.add_keyboard_callback(lazy.carb.input.KeyboardInput.B, lambda: (self._mark_control_key(), self._bump_rpy(-self.rot_step, 0, 0)))
        KeyboardEventHandler.add_keyboard_callback(lazy.carb.input.KeyboardInput.O, lambda: (self._mark_control_key(), self._bump_rpy(0, +self.rot_step, 0)))
        KeyboardEventHandler.add_keyboard_callback(lazy.carb.input.KeyboardInput.U, lambda: (self._mark_control_key(), self._bump_rpy(0, -self.rot_step, 0)))
        KeyboardEventHandler.add_keyboard_callback(lazy.carb.input.KeyboardInput.V, lambda: (self._mark_control_key(), self._bump_rpy(0, 0, +self.rot_step)))
        KeyboardEventHandler.add_keyboard_callback(lazy.carb.input.KeyboardInput.C, lambda: (self._mark_control_key(), self._bump_rpy(0, 0, -self.rot_step)))
        ...
        KeyboardEventHandler.add_keyboard_callback(lazy.carb.input.KeyboardInput.G, self._cli_goal_prompt)
        # Paste object pose as JSON with keys R (3x3) and t (3,) in CAMERA frame
        KeyboardEventHandler.add_keyboard_callback(lazy.carb.input.KeyboardInput.K, self._cli_object_pose_json)

        # Base nudges: W/S forward/back (x), A/D strafe left/right (y), Q/E rotate CCW/CW (rz)
        KeyboardEventHandler.add_keyboard_callback(
            lazy.carb.input.KeyboardInput.W, lambda:(self._mark_control_key(), self._nudge_base(+self.base_lin_step, 0.0, 0.0))
        )
        KeyboardEventHandler.add_keyboard_callback(
            lazy.carb.input.KeyboardInput.S, lambda: (self._mark_control_key(), self._nudge_base(-self.base_lin_step, 0.0, 0.0))
        )
        KeyboardEventHandler.add_keyboard_callback(
            lazy.carb.input.KeyboardInput.A, lambda: (self._mark_control_key(),self._nudge_base(0.0, +self.base_lin_step, 0.0))
        )
        KeyboardEventHandler.add_keyboard_callback(
            lazy.carb.input.KeyboardInput.D, lambda: (self._mark_control_key(), self._nudge_base(0.0, -self.base_lin_step, 0.0))
        )
        KeyboardEventHandler.add_keyboard_callback(
            lazy.carb.input.KeyboardInput.Q, lambda: (self._mark_control_key(), self._nudge_base(0.0, 0.0, +self.base_yaw_step))
        )
        KeyboardEventHandler.add_keyboard_callback(
            lazy.carb.input.KeyboardInput.E, lambda: (self._mark_control_key(), self._nudge_base(0.0, 0.0, -self.base_yaw_step))
        )
        # Head tilt (joint-based, pitch)
        KeyboardEventHandler.add_keyboard_callback(
            lazy.carb.input.KeyboardInput.PAGE_UP, lambda: (self._mark_control_key(), self._tilt_head(+float(self.head_pitch_step)))
        )
        KeyboardEventHandler.add_keyboard_callback(
            lazy.carb.input.KeyboardInput.PAGE_DOWN, lambda: (self._mark_control_key(), self._tilt_head(-float(self.head_pitch_step)))
        )
        # Head turn (joint-based, yaw) left / right
        # Use J (left) and L (right) to avoid overlap with EEF keys
        KeyboardEventHandler.add_keyboard_callback(
            lazy.carb.input.KeyboardInput.J, lambda: (self._mark_control_key(), self._yaw_head(+float(self.head_yaw_step)))
        )
        KeyboardEventHandler.add_keyboard_callback(
            lazy.carb.input.KeyboardInput.L, lambda: (self._mark_control_key(), self._yaw_head(-float(self.head_yaw_step)))
        )
        
        # Use Y to toggle XVLA autopilot mode
        KeyboardEventHandler.add_keyboard_callback(
            lazy.carb.input.KeyboardInput.Y, self._toggle_xvla_mode
        )

    def _quat_to_rot6d(self, quat_xyzw: th.Tensor) -> th.Tensor:
        """
        quat_xyzw: (4,)
        returns rot6d: (6,) = first two columns of rotation matrix
        """
        R = T.quat2mat(quat_xyzw)  # (3,3)
        return th.cat([R[:, 0], R[:, 1]], dim=0)

    def _rot6d_to_quat(self, rot6d: th.Tensor) -> th.Tensor:
        
        """
        rot6d: (6,) -> quat_xyzw: (4,)
        Uses Gram-Schmidt to form SO(3).
        """
        a1 = rot6d[0:3]
        a2 = rot6d[3:6]
        b1 = a1 / (th.linalg.norm(a1) + 1e-8)
        a2 = a2 - th.dot(b1, a2) * b1
        b2 = a2 / (th.linalg.norm(a2) + 1e-8)
        b3 = th.cross(b1, b2)
        R = th.stack([b1, b2, b3], dim=1)  # (3,3) columns
        return T.mat2quat(R)  # xyzw

    def _toggle_xvla_mode(self):
        if self.xvla_client is None:
            print("[XVLA] Client not configured.")
            return
        self.xvla_mode = not self.xvla_mode
        self._xvla_action_queue.clear()
        self._xvla_capture_pending = False
        self._xvla_error = None
        print(f"[XVLA] Mode = {self.xvla_mode}")
        
    
    def _get_xvla_inputs(self):
        # image (head rgb)
        ...
        # proprio must be length 20 for ee6d action_space (gripper_idx=(9,19))
        # Layout: [L xyz(3), L rot6d(6), L grip(1), R xyz(3), R rot6d(6), R grip(1)] = 20
        dtype = th.float32
        device = self.desired[self.active_arm][0].device
        proprio = np.zeros(20, dtype=np.float32)

        # left
        posL, quatL = self._get_eef_pose("left")
        rot6dL = self._quat_to_rot6d(quatL)
        proprio[0:3] = posL
        proprio[3:9] = rot6dL
        proprio[9] = self._get_gripper_scalar("left")

        # right
        posR, quatR = self._get_eef_pose("right")
        rot6dR = self._quat_to_rot6d(quatR)
        proprio[10:13] = posR
        proprio[13:19] = rot6dR
        proprio[19] = self._get_gripper_scalar("right")

        return proprio, rgb
    def _get_gripper_scalar(self, arm: str) -> float:
        cmd = self._gripper_hold_command.get(arm, None)
        if cmd is None:
            return 0.0
        return float(cmd.mean().item())
    def _handle_xvla_actions(self, actions_np: np.ndarray):
        # actions_np: (T,20)
        if actions_np.ndim != 2 or actions_np.shape[1] != 20:
            raise ValueError(f"XVLA returned {actions_np.shape}, expected (T,20)")
        self._xvla_action_queue.clear()
        for t in range(actions_np.shape[0]):
            self._xvla_action_queue.append(th.from_numpy(actions_np[t]).float())
        print(f"[XVLA] Received {actions_np.shape[0]} actions -> queued.")
    ...
    def _trigger_xvla_request(self):
        if self.xvla_client is None:
            print("[XVLA] Client not configured.")
            return
        if self._xvla_inflight:
            print("[XVLA] Request already in flight.")
            return
        self._xvla_capture_pending = True
        print("[XVLA] Capture queued; will send on next control tick.")


    def _start_xvla_request(self):
        self._xvla_capture_pending = False
        if self.xvla_client is None:
            return

        try:
            proprio, rgb = self._get_xvla_inputs()
        except Exception as e:
            self._xvla_error = f"Failed to capture inputs: {e}"
            return

        instruction = self._xvla_instruction or "Do the task"
        domain_id = self._xvla_domain_id
        steps = self._xvla_steps

        self._xvla_inflight = True

        def _worker():
            try:
                actions_np = self.xvla_client.act(
                    proprio=proprio,
                    image0=rgb,
                ...
                # actions_np: (T, action_dim_policy)
                self._handle_xvla_actions(actions_np)
            except Exception as exc:
                self._xvla_error = str(exc)
            finally:
                self._xvla_inflight = False
    ...
    def _bind_head_tilt_joint(self) -> Optional[Tuple[str, int, str]]:
        ...
        print("[Teleop] Unable to bind a head tilt joint; verify robot has a head/torso tilt DOF.")
        return None
    ...
    def _bind_head_yaw_joint(self) -> Optional[Tuple[str, int, str]]:
        ...
        for j_idx in candidates:
            ...
            return comp, start + local, joint_name
        print("[Teleop] Unable to bind a head yaw joint; verify robot has a head/torso yaw DOF.")
        return None
    ...
    def _compute_intrinsics(self, sensor, sensor_name: Optional[str], rgb_shape: Tuple[int, int]) -> np.ndarray:
        intrinsics = None
        ...
        for attempt in range(6):
            try:
                ...
                if proj is not None and resolution is not None:
                    proj_np = proj.detach().cpu().numpy() if hasattr(proj, "detach") else np.asarray(proj)
                    res_np = resolution.detach().cpu().numpy() if hasattr(resolution, "detach") else np.asarray(resolution)
                ...
                intrinsics = None
            if intrinsics is not None and np.all(np.isfinite(intrinsics)) and np.any(intrinsics) and intrinsics[0,0] > 0 and intrinsics[1,1] > 0:
                break
            try:
                og.sim.render()
            except Exception:
        ...
        if (intrinsics is None or not np.all(np.isfinite(intrinsics)) or not np.any(intrinsics)):
            camera_id = None
            ...
            if intrinsics is None or not np.all(np.isfinite(intrinsics)) or not np.any(intrinsics):
                ...
                if not self._sam6d_intrinsics_warned:
                    print("[SAM-6D] Falling back to identity intrinsics; camera parameters unavailable.")
                    self._sam6d_intrinsics_warned = True
        return intrinsics
    
    def _save_action(self, action: th.Tensor):
        a = action.detach().cpu().tolist()
        self._action_f.write(json.dumps(a) + "\n")
        ...
        if obj is not None:
            ...
            if category and model:
                ...
                if refined.lower() != self._sam6d_target_object.lower():
                    ...
                    print(f"[SAM-6D] Refined SAM-6D target object to '{self._sam6d_target_object}'.")
        if obj is None:
            if not self._sam6d_warned_object:
                print(f"[SAM-6D] Could not find object named '{self._sam6d_target_object}' in the scene.")
    ...
    def _capture_head_data(self, label: str) -> Optional[Dict[str, object]]:
        ...
        if missing:
            print(f"[{label}] Missing modalities {missing}. Update robot obs_modalities to include them.")
            return None
        rgb = self._tensor_to_numpy(obs["rgb"])
        if rgb.shape[-1] == 4:
            rgb = rgb[..., :3]
        if np.issubdtype(rgb.dtype, np.floating):
            ...
            rgb = rgb.astype(np.uint8, copy=False)
        depth = self._tensor_to_numpy(obs["depth_linear"]).astype(np.float32, copy=False)
        semantic = self._tensor_to_numpy(obs["seg_semantic"]).astype(np.int32, copy=False)
        intrinsics = self._compute_intrinsics(sensor, sensor_name, (rgb.shape[0], rgb.shape[1]))
        cam_link = self._camera_link_from_sensor_name(sensor_name) if sensor_name is not None else None
        if cam_link is None:
            cam_link = "zed_link"
        ...
        cam_quat_np = self._tensor_to_numpy(cam_quat_base).astype(np.float32, copy=False)
    ...
    def _begin_auto_drive(self, target_pos: th.Tensor, target_quat: th.Tensor, label: str = "target"):
        ...
        #self._drive_best_pos_err = float("inf")
        #self._drive_best_ori_err = float("inf")
        self._last_ik_action = None
    ...
    def _maybe_report_drive_progress(self):
        ...
        # calculate avg orientation error among three axes
        #ori_delta = self.quat_angular_difference(cur_quat, tgt_quat)
        # print("zzzzzzzzzzzzzzzzzzz",cur_quat, tgt_quat)
        # print("zzzzzzzzzzzzzzzzzzz",T.quat2axisangle(cur_quat), T.quat2axisangle(tgt_quat))
        ori_delta = th.mean(th.abs(T.quat2axisangle(cur_quat) - T.quat2axisangle(tgt_quat)))
        print(f"[Drive] {self._drive_label}: t={now - self._drive_start_time:.2f}s cur_rot={cur_quat.tolist()} tgt_rot={tgt_quat.tolist()}")
        ...
        print(f"[Drive] {self._drive_label}: t={elapsed:.2f}s pos_err={pos_err:.4f}m ori_err={th.rad2deg(th.tensor(ori_err)).item():.2f}deg")

    def _evaluate_drive_goal(self, now: float, cur_pos: Optional[th.Tensor] = None, cur_quat: Optional[th.Tensor] = None) -> None:
        if not self._drive_active or self._drive_target is None:
            return
        ...
        print(f"[Drive in step] {self._drive_label}: pos_err={pos_err:.4f}m ori_err={th.rad2deg(th.tensor(ori_err)).item():.2f}deg stagnation={stagnation:.2f}s")
        if pos_ok and ori_ok:
            print(f"[EEF exec] {self.active_arm}: pos={cur_pos.tolist()} quat(xyzw)={cur_quat.tolist()} (reached)")
            self._drive_active = False
            self._drive_target = None
            self._last_ik_action = None
            self._target_dirty = False
        ...
        if pos_ok and stagnation > 1.0:
            print(f"[Drive] Position reached but orientation stagnant for {stagnation:.2f}s; accepting pose.")
            self._drive_active = False
            self._drive_target = None
            self._last_ik_action = None
            self._target_dirty = False
            ...
            print(f"[Drive] Timeout ({elapsed:.1f}s). Leaving auto mode (pos_err={pos_err:.4f} ori_err={th.rad2deg(th.tensor(ori_err)).item():.2f}deg)")
            self._drive_active = False
            self._drive_target = None
            self._last_ik_action = None
            self._target_dirty = False
    ...
    def _schedule_target_from_camera_Rt(self, R_cam_obj: np.ndarray, t_cam_obj: np.ndarray, arm: Optional[str] = None,
                                        approach_offset_m: float = 0.0, source: str = "camera",
    ...
    def _apply_scheduled_cam_target(self):
        ...
        try:
            ...
            if cam_pose_override is not None:
                ...
                cam_pos_base = th.tensor(cam_pose_override[0], dtype=dtype, device=device)
                cam_quat_base = th.tensor(cam_pose_override[1], dtype=dtype, device=device)
            else:
                name, sensor = self._get_head_sensor()
                cam_link = self._camera_link_from_sensor_name(name) if name is not None else None
                if cam_link is None:
                    cam_link = "zed_link"
                cam_pos_base, cam_quat_base = get_pose_in_base(self.robot, cam_link)
            T_base_cam = T.pose2mat((cam_pos_base, cam_quat_base))
            # Sanitize R to SO(3)
            R = th.tensor(np.asarray(R_cam_obj), dtype=cam_pos_base.dtype, device=cam_pos_base.device)
            ...
            t = th.tensor(np.asarray(t_cam_obj), dtype=cam_pos_base.dtype, device=cam_pos_base.device)
            # if approach_offset_m != 0.0:
            #     offset = R_hat @ th.tensor([0.0, 0.0, -float(approach_offset_m)], dtype=R_hat.dtype, device=R_hat.device)
            #     t = t + offset
            # Apply EEF orientation correction (align camera frame with gripper frame)
            #R_hat = R_hat @ self._eef_rot_correction.to(R_hat)
            # Build homogeneous mat for cam_T_obj
            ...
            try:
                if self._base_link_name and self._base_link_name in self.robot.links:
                    ...
                    base_pos_world, base_quat_world = base_link.get_position_orientation()
                else:
                    base_pos_world, base_quat_world = self.robot.get_position_orientation()
                ...
                print(f"[Grasp viz] {arm} {source}: world pos={world_pos.tolist()} quat={world_quat.tolist()}")
            except Exception as exc:
    ...
    def _set_grasp_viz_pose(self, arm: str, pos_base: th.Tensor, quat_base: th.Tensor, source: str = "") -> None:
        if not self._grasp_viz_enabled:
            return
        try:
            self._grasp_viz_pose[arm] = (pos_base.detach().clone(), quat_base.detach().clone())
    ...
    @staticmethod
    def _scale_axis_color(color: Tuple[float, float, float, float], active: bool) -> Tuple[float, float, float, float]:
        if active:
            return color
        dim = 0.45
        ...
        try:
            ...
            for arm, pose in self._grasp_viz_pose.items():
                ...
                for idx, color in enumerate(axis_colors):
                    ...
                    draw_line(origin.tolist(), end.tolist(), color=self._scale_axis_color(color, active), size=self._grasp_viz_line_size if active else self._grasp_viz_line_size * 0.75)
                approach_dir = -axes[:, 2]
                approach_end = origin + approach_dir * (self._grasp_viz_line_scale * 0.6)
                approach_color = (1.0, 0.8, 0.25, 1.0) if active else (0.6, 0.5, 0.25, 1.0)
                draw_line(origin.tolist(), approach_end.tolist(), color=approach_color, size=self._grasp_viz_line_size if active else self._grasp_viz_line_size * 0.75)
                drawn_any = True
    ...
    def _get_head_sensor(self):
        ...
        for name, sensor in self.robot.sensors.items():
            if hasattr(sensor, "get_obs") and ("zed" in name):
                return name, sensor
        for name, sensor in self.robot.sensors.items():
            if hasattr(sensor, "get_obs") and ("Camera" in name) and ("realsense" not in name):
                return name, sensor
    ...
    def _print_current_pose_and_joints(self):
        ...
        print(f"Current {arm} EEF (base): pos={pos.tolist()} quat(xyzw)={quat.tolist()}")
        names = list(self.robot.joints.keys())
        q = self.robot.get_joint_positions()
    ...
    def _cli_object_pose_json(self):
        ...
        try:
            s = input("JSON {\"R\": [[...],[...],[...]], \"t\": [x,y,z]} (blank to cancel): ").strip()
        except EOFError:
            s = ""
        if not s:
            print("Cancelled.")
        ...
        try:
            ...
            if R.shape != (3, 3) or t.shape != (3,):
                print(f"Invalid shapes: R{R.shape}, t{t.shape}. Expected (3,3) and (3,).")
                return
            # Schedule application in the main loop to avoid heavy operations inside the key callback
            self._schedule_target_from_camera_Rt(R, t, arm=self.active_arm, approach_offset_m=0.0, source="cli")
    ...
    def step(self) -> th.Tensor:
        ...
        # 0) Manual one-shot action override should ALWAYS work
        if self._pending_action is not None:
            act = self._pending_action
            self._pending_action = None
            print("action:", act.tolist())

            if self._control_key_pressed:
                self._save_action(act)
            self._control_key_pressed = False

            return act
        
         # 1) XVLA autopilot updates desired pose BEFORE IK logic
        if self.xvla_mode:
            if (not self._xvla_inflight) and (len(self._xvla_action_queue) == 0):
                self._xvla_capture_pending = True

            if self._xvla_capture_pending and (not self._xvla_inflight):
                self._start_xvla_request()

            if len(self._xvla_action_queue) > 0:
                a = self._xvla_action_queue.popleft()  # (20,)
                # Convert to torch on the right device
                device = self.desired[self.active_arm][0].device
                a_th = th.as_tensor(a, dtype=th.float32, device=device)
                # Left
                posL = a_th[0:3]
                r6L  = a_th[3:9]
                gL   = float(a_th[9].item())
                quatL = self._rot6d_to_quat(r6L)

                # Right
                posR = a_th[10:13]
                r6R  = a_th[13:19]
                gR   = float(a_th[19].item())
                quatR = self._rot6d_to_quat(r6R)

                # Update BOTH desired targets
                self.desired["left"]  = (posL, quatL)
                self.desired["right"] = (posR, quatR)

                # Mark dirty so IK will run
                self._target_dirty = True

                # Store both grippers (0..1)
                if not hasattr(self, "_xvla_grip_target"):
                    self._xvla_grip_target = {"left": None, "right": None}
                self._xvla_grip_target["left"] = gL
                self._xvla_grip_target["right"] = gR
            
            
        # 2) proceed with your existing action building + IK
        action = th.zeros(self.robot.action_dim)
        ...
        targets_L = self.ik.solve_to_joint_targets(arm="left",  target_pose_base=self.desired["left"])
        targets_R = self.ik.solve_to_joint_targets(arm="right", target_pose_base=self.desired["right"])

        # If both failed, do nothing this step
        if targets_L is None and targets_R is None:
            return action
        ...
        def apply_targets_to_group(grp: str, targets: Dict[str, float], trunk_add: bool = False):
            if grp not in self.controller_info:
                return
            start = self.controller_info[grp]["start_idx"]
            dof_idx = self.controller_info[grp]["dofs"].tolist()
            for local_i, j_idx in enumerate(dof_idx):
                ...
                if j_name in targets:
                    dq = float(targets[j_name]) - float(q[j_idx])  # abs -> delta
                    if trunk_add:
                        action[start + local_i] += dq
                    else:
                        action[start + local_i] = dq

        # TRUNK: apply from ONE side only (avoid conflicts)
        if targets_L is not None:
            apply_targets_to_group("trunk", targets_L, trunk_add=True)
        elif targets_R is not None:
            apply_targets_to_group("trunk", targets_R, trunk_add=True)

        # Arms: apply both if available
        if targets_L is not None:
            apply_targets_to_group("arm_left", targets_L, trunk_add=False)
        if targets_R is not None:
            apply_targets_to_group("arm_right", targets_R, trunk_add=False)

        # Apply head nudges on top of IK trunk commands (keep your original logic)
        if pitch_idx is not None:
            action[pitch_idx] += pitch_val
        if yaw_idx is not None:
            action[yaw_idx] += yaw_val

        # (optional) print both EE poses
        cur_pos_L, cur_quat_L = self._get_eef_pose("left")
        cur_pos_R, cur_quat_R = self._get_eef_pose("right")
        print(f"[EEF after IK] left : pos={cur_pos_L.tolist()} quat={cur_quat_L.tolist()}")
        print(f"[EEF after IK] right: pos={cur_pos_R.tolist()} quat={cur_quat_R.tolist()}")

        # Clear accumulators / bookkeeping (keep same as yours)
        self._head_nudge = 0.0
        self._head_yaw_nudge = 0.0
        ...
        # If your _evaluate_drive_goal assumes active_arm only, either:
        # - call it for active_arm only, OR
        # - skip it in xvla_mode, OR
        # - add a second evaluation for right arm
        self._evaluate_drive_goal(now)
...
def gently_tilt_head_down(robot, joint_name: str = "torso_joint4", offset_rad: float = -0.15) -> None:
    ...
    try:
        ...
        robot.set_joint_positions(positions=target.unsqueeze(0), indices=th.tensor([j_idx]), drive=False)
    except Exception:
        # Best-effort; ignore if robot API differs
        pass


def fully_open_grippers(robot, margin_ratio: float = 1e-3, min_margin: float = 1e-4, verbose: bool = True) -> None:
    ...
    for arm in getattr(robot, "arm_names", []):
        ...
        try:
            ...
            robot.set_joint_positions(positions=pos_tensor, indices=idx_tensor, drive=False)
            any_opened = True
        except Exception:
            continue
...
def main():
    parser = argparse.ArgumentParser(description="R1Pro whole-body IK teleop (base-frame EEF control)")
    ...
    parser.add_argument("--action-jsonl",type=str,default="teleop_actions.jsonl",help="Path to write per-step actions as JSONL",)
    ...
    xvla_client = XVLAHttpClient("http://localhost:8000/act", timeout=10.0)

    teleop = EEFBaseTeleop(
        robot,
        env=env,
        action_jsonl_path=args.action_jsonl,
        xvla_client=xvla_client,
        xvla_instruction='turning on radio',
        xvla_domain_id=0,
        xvla_steps=10
    )
    ...
    print("K: paste object R (3x3) and t (3,) in CAMERA frame (JSON) — gripper goes to object")
