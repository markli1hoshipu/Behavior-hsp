...
class PolicyEvalDataCollectionWrapper(HDF5CollectionWrapper):
    ...
    def _optimize_sim_for_data_collection(self, viewport_camera_path):
        ...
        # Set the dump filter for better performance (keep this from original)
...
class Evaluator:
    ...
    def load_env(self, env_wrapper: DictConfig) -> EnvironmentWrapper:
        ...
        # Wrap with DataCollectionWrapper if recording_path is specified
        record_path = getattr(self.cfg, "recording_path", None)
        if record_path is not None:
            record_path = Path(record_path).expanduser()
            logger.info(f"Recording successful rollouts to: {record_path}")
    ...
    def _finalize_episode_recording(self, instance_id: int, episode_idx: int) -> None:
        """
        Flush the current trajectory to disk (if enabled) and tag it with metadata.
        """
        if self.data_recorder is None:
            return
        prev_traj_count = self.data_recorder.traj_count
        self.data_recorder.flush_current_traj()
        if self.data_recorder.traj_count > prev_traj_count:
            demo_idx = self.data_recorder.traj_count - 1
            demo_group = self.data_recorder.hdf5_file["data"][f"demo_{demo_idx}"]
            self.data_recorder.add_metadata(group=demo_group, name="task_name", data=self.cfg.task.name)
            self.data_recorder.add_metadata(group=demo_group, name="instance_id", data=instance_id)
            self.data_recorder.add_metadata(group=demo_group, name="episode_idx", data=episode_idx)

    def _finalize_data_collection(self) -> None:
        """
        Flush any pending data and close the recorder if it is enabled.
        """
        if self.data_recorder is None:
            return
        if len(self.data_recorder.current_traj_history) > 0:
            self.data_recorder.flush_current_traj()
...
if __name__ == "__main__":
    ...
    # set default recording path (for successful rollout hdf5 logs)
    if config.recording_path is None:
        traj_dir = Path(config.log_path).expanduser() / "trajectories"
    ...
    with Evaluator(config) as evaluator:
        ...
        for idx in instances_to_run:
            ...
            for epi in range(m.NUM_EVAL_EPISODES):
                ...
                evaluator._finalize_episode_recording(instance_id=int(idx), episode_idx=epi)
